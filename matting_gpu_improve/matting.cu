...
Mat image, image_gray, mask, trimap, indexing, result;
struct UnknownPoint TU[MaxSize];

GpuMat GpuImage, GpuImage_gray, GpuMask, GpuTrimap, GpuIndexing, GpuResult;
...
__global__ void PostProcess(PtrStepSz<uchar> Mask, PtrStepSz<uchar> Result);
...
int main(void) {
    ...
    for (int n = 1; n < 65; n++) {
		...
		// PostProcess<<<ceil(1.0 * (height / 2) * width / 1024), 1024>>>(GpuMask, GpuResult);
...
__global__ void SampleGathering1(struct UnknownPoint *TU, PtrStepSz<uchar3> Image, PtrStepSz<uchar> Image_gray, PtrStepSz<uchar> Trimap, PtrStepSz<uchar> Result) {
	...
	for (int k = 0; k < KG; k++) {
		...
		int i1 = max(0, i - 100);
		int i2 = min(i + 100, height - 1);
		int j1 = max(0, j - 100);
		int j2 = min(j + 100, width - 1);
...
__global__ void PostProcess(PtrStepSz<uchar> Mask, PtrStepSz<uchar> Result) {
	...
	if(i >= height / 2 || i < 0 || j >= width || j < 0)
		return;

	uchar gray1 = Mask(i, j);
	uchar gray2 = Result(i, j);
